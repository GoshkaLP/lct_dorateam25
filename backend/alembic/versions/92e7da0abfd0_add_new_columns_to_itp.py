"""add new columns to itp

Revision ID: 92e7da0abfd0
Revises: 0fc9359b4c87
Create Date: 2025-10-04 20:40:16.786031
...
revision: str = '92e7da0abfd0'
down_revision: Union[str, Sequence[str], None] = '0fc9359b4c87'
...
def upgrade() -> None:
    ...
    op.add_column('itp', sa.Column('status', sa.Enum('ORANGE', 'YELLOW', 'GREEN', 'UNKNOWN', name='objectstatus', native_enum=False), nullable=True))
    op.add_column('itp', sa.Column('layout_index', sa.Integer(), nullable=True))
    op.drop_constraint(op.f('itp_mkd_id_mkd_fkey'), 'itp_mkd', type_='foreignkey')
    op.drop_constraint(op.f('itp_mkd_id_itp_fkey1'), 'itp_mkd', type_='foreignkey')
...
def downgrade() -> None:
    ...
    op.drop_constraint(None, 'itp_mkd', schema='lctdora', type_='foreignkey')
    op.create_foreign_key(op.f('itp_mkd_id_itp_fkey1'), 'itp_mkd', 'itp', ['id_itp'], ['id'])
    op.create_foreign_key(op.f('itp_mkd_id_mkd_fkey'), 'itp_mkd', 'mkd', ['id_mkd'], ['id'])
    op.drop_column('itp', 'layout_index')
